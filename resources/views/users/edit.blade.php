...
<div class="max-w-2xl mx-auto bg-zinc-800/60 border border-zinc-700/60 p-8 rounded-xl shadow-xl backdrop-blur-sm">
    <div class="mb-6">
        <h2 class="text-xl font-bold text-white tracking-tight">Edit User: {{ $user->name }}</h2>
        <p class="text-sm text-zinc-400 mt-1">Update account details, role permissions, or password</p>
    </div>
        ...
        <div>
            ...
            <select name="role_id" required
                ...
                @foreach ($roles as $role)
                    <option value="{{ $role->id }}" class="bg-zinc-900 text-white" {{ old('role_id', $user->role_id) == $role->id ? 'selected' : '' }}>
                        {{ $role->name }}
                    </option>
                @endforeach
            </select>
