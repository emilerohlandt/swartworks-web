...
@section('title', 'Add Model Hub')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-white tracking-tight">Add Model Hub</h1>
            <p class="text-sm text-zinc-400 mt-1">Configure a new external 3D model repository link.</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.model-hubs.store') }}" enctype="multipart/form-data" class="bg-zinc-800/60 border border-zinc-700/60 rounded-xl shadow-xl p-6 backdrop-blur-sm space-y-6">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3" required
                      ...
                      class="w-full bg-zinc-900 border border-zinc-700/80 rounded-lg px-3.5 py-2.5 text-sm text-white placeholder-zinc-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-xs text-red-400 mt-1">{{ $message }}</p>
            @enderror
        </div>
