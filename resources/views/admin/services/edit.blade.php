...
<div class="max-w-2xl mx-auto bg-zinc-800/60 border border-zinc-700/60 p-8 rounded-xl shadow-xl backdrop-blur-sm">
    <div class="mb-6">
        <h2 class="text-xl font-bold text-white tracking-tight">Edit Service: {{ $service->name }}</h2>
        <p class="text-sm text-zinc-400 mt-1">Update service parameters, callout badge, assigned materials, or primary image</p>
    </div>
    ...
    <form method="POST" action="{{ route('admin.services.update', $service) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                      class="mt-1.5 w-full px-3.5 py-2.5 bg-zinc-900 border border-zinc-700 rounded-lg text-white placeholder-zinc-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors text-sm">{{ old('description', $service->description) }}</textarea>
            @error('description')
                <span class="text-red-400 text-xs mt-1 block">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" name="callout_text" value="{{ old('callout_text', $service->callout_text) }}" placeholder="e.g. Standard & Tough"
                   class="mt-1.5 w-full px-3.5 py-2.5 bg-zinc-900 border border-zinc-700 rounded-lg text-white placeholder-zinc-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-colors text-sm">
                ...
                @foreach ($materials as $material)
                    <label class="flex items-center space-x-2.5 text-sm text-zinc-300 bg-zinc-900/80 p-3 rounded-lg border border-zinc-700/80 cursor-pointer hover:bg-zinc-700/40 hover:border-zinc-600 transition-colors">
                        <input type="checkbox" name="materials[]" value="{{ $material->id }}"
                            {{ in_array($material->id, old('materials', $service->materials->pluck('id')->toArray())) ? 'checked' : '' }}
        ...
        <div>
            ...
            @if ($service->image_path)
                <div class="mb-3 relative w-48 h-28 rounded-lg overflow-hidden border border-zinc-700 bg-zinc-900">
                    <img src="{{ Storage::url($service->image_path) }}" alt="{{ $service->name }}" class="w-full h-full object-cover">
                    @if ($service->callout_text)
                        <span class="absolute top-2.5 left-2.5 px-2.5 py-1 text-[11px] font-semibold bg-zinc-950/80 text-amber-400 border border-amber-500/30 rounded-full backdrop-blur-xs">
                            {{ $service->callout_text }}
                        </span>
                    @endif
                </div>
            @endif
